-- Insert OnShelf AI Extraction Prompts without ON CONFLICT
-- First check if prompts already exist, then insert only new ones

-- Check if our prompts already exist
...
-- Insert prompts only if they don't exist
DO $$
BEGIN
...
{/IF_RETRY}',
    ...
    -- (Adding just a few more for brevity)
...
SELECT 'Checking inserted prompts:' as status;
SELECT 
    template_id,
    name,
    stage_type,
