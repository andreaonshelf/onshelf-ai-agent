...
-- FIXED: Better handling of duplicate names
...
-- Step 12: Fix duplicate names more intelligently
...
BEGIN
    ...
    FOR dup_record IN 
        SELECT name, stage_type, COUNT(*) as dup_count
    ...
    LOOP
        ...
        SET name = p.name || ' (v' || ROW_NUMBER() OVER (ORDER BY p.created_at, p.prompt_id) || ')'
        FROM (
            SELECT prompt_id, created_at
            FROM prompt_templates
            WHERE name = dup_record.name 
            AND stage_type = dup_record.stage_type
            ORDER BY created_at, prompt_id
        ) dup
        WHERE p.prompt_id = dup.prompt_id;
    END LOOP;
END $$;

-- Now check again and if still duplicates, use prompt_id as last resort
WITH duplicates AS (
...
UPDATE prompt_templates p
SET name = p.name || ' (' || LEFT(p.prompt_id::TEXT, 8) || ')'
FROM duplicates d
WHERE p.name = d.name 
