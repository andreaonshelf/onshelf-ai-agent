-- SAFE migration to update prompt_templates table to support Pydantic field schemas
-- This migration PRESERVES BACKWARD COMPATIBILITY by keeping prompt_content column
...
-- Step 6: DO NOT RENAME prompt_content! Instead, add prompt_text as a copy
-- This maintains backward compatibility
ALTER TABLE prompt_templates 
ADD COLUMN IF NOT EXISTS prompt_text TEXT;

-- Copy content from prompt_content to prompt_text
UPDATE prompt_templates 
SET prompt_text = prompt_content
WHERE prompt_text IS NULL AND prompt_content IS NOT NULL;
...
-- Step 12: Create a trigger to keep prompt_content and prompt_text in sync
-- This ensures backward compatibility while supporting new code
CREATE OR REPLACE FUNCTION sync_prompt_content_text()
RETURNS TRIGGER AS $$
BEGIN
    -- If prompt_text is updated, update prompt_content
    IF TG_OP = 'UPDATE' OR TG_OP = 'INSERT' THEN
        IF NEW.prompt_text IS NOT NULL AND (NEW.prompt_content IS NULL OR NEW.prompt_text != COALESCE(OLD.prompt_text, '')) THEN
            NEW.prompt_content = NEW.prompt_text;
        ELSIF NEW.prompt_content IS NOT NULL AND (NEW.prompt_text IS NULL OR NEW.prompt_content != COALESCE(OLD.prompt_content, '')) THEN
            NEW.prompt_text = NEW.prompt_content;
        END IF;
    END IF;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS sync_prompt_content_text_trigger ON prompt_templates;

CREATE TRIGGER sync_prompt_content_text_trigger
BEFORE INSERT OR UPDATE ON prompt_templates
FOR EACH ROW
EXECUTE FUNCTION sync_prompt_content_text();

-- Step 13: Add constraint to ensure name is unique within stage_type
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'prompt_templates_name_stage_type_unique'
    ) THEN
        ALTER TABLE prompt_templates 
...
COMMENT ON COLUMN prompt_templates.prompt_text IS 'New column name for prompt content (synced with prompt_content for backward compatibility)';
COMMENT ON COLUMN prompt_templates.prompt_content IS 'Legacy column name (synced with prompt_text for backward compatibility)';
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'prompt_templates' AND column_name = 'prompt_content') THEN
        missing_columns := array_append(missing_columns, 'prompt_content (CRITICAL - old code needs this!)');
    END IF;
    ...
    ELSE
        RAISE NOTICE 'Safe migration completed successfully. Both prompt_content and prompt_text columns exist.';
    END IF;
END $$;
